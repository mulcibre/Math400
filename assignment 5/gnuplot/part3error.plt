...
set output "a5p3erroroutput.png"

#   set title, labels, key position
set title "Approximation Error of Legendre Polynomials"
set xlabel "<-- (x) -->"
set ylabel "<-- (y) -->"
...
#   Plot piecewise, and errors for the different ordered polynomial approximations
plot a(x) , \
abs(a(x) - f1(x)), \
abs(a(x) - f2(x)), \
abs(a(x) - f3(x)), \
